...
class DeribitWebSocketClient:
    ...
    async def send_request(
        ...
        try:
            ...
            logging.error(f"API Error: Code {e.code}, Message: {e.message}")
            raise
        except Exception as e:
            self.pending_requests.pop(message_id, None)
            logging.error(
    ...
    # Helper method to provide a simple way to run the client
    @classmethod
    async def run_client(cls,
                         client_id: Optional[str] = None,
                         client_secret: Optional[str] = None,
                         use_test_env: bool = True) -> None:
        """
        Create and run a client instance with a simple example.
